...
console.log(gl_hr);
 let str = `
1. РЕКОМЕНДУЕТСЯ ПРИМЕНЯТЬ КОНТРОЛЛЕР AS СИНТАКСИС И КАК МОЖНО РЕЖЕ ИСПОЛЬЗОВАТЬ $scope!!!
2. В HTML <div ng-controller="firstCtrl as firstCtrl">
3. В controller ВМЕСТО $scope ИСПОЛЬЗУЕТСЯ this.
4. БЛАГОДОРЯ ЭТОМУ В HTML КОНКРЕТЕЗИРУЮТСЯ ПЕРЕМЕННЫЕ КОНТРОЛЛЕРОВ - {{mainCtrl.myLesson}}
 `
 console.log(str + '\n' + gl_hr);
