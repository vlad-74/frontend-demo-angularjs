...
console.log(gl_hr);
 let str = `
1. transclude: 'element' - ПОЗВОЛЯЕТ ДЕЛАТЬ transclude ВСЕЙ ДИРЕКТИВЫ СРАЗУ
2. ГЕНЕРИРУЕМ ЧАСТЬ ВНЕ ДИРЕКТИВЫ (В ШАБЛОНЕ) А ЗАТЕМ ДОПИСЫВАЕМ ДИРЕКТИВУ КАК НАМ УДОБНО
3. ОБОРАЧИВАЕМ ДИРЕКТИВУ В НУЖНЫЙ КОНТЕНТ!
