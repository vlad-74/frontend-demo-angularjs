...
console.log(gl_hr);
 let str = `
1. В HTML СОЗДАТЬ ЭЛЕМЕНТ = <foo-bar></foo-bar>
2. В ДИРЕКТИВЕ СОЗДАТЬ МАССИВ ОБЪЕКТОВ:
2.1. var bookmarks = [{id: 1,name: 'AngularJS'},{id: 2,name: 'EmberJS'}];
3. В link ДИРЕКТИВЫ ДОБАВИТЬ : scope.myBookmarks = bookmarks;
4. В template ДИРЕКТИВЫ :
4.1. template: "<div ng-repeat='bookmark in myBookmarks'>{{bookmark.name}}</div>"
 `
 console.log(str + '\n' + gl_hr);
