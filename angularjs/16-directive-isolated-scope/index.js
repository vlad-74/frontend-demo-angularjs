...
console.log(gl_hr);
 let str = `
1. scope: {} - ИЗОЛИРОВАННЫЙ СКОУП - ЗАДАТЬ СКОУП КАК ОБЪЕКТ
2. В ИЗОЛИРОВАННОМ СКОУП НЕТ ПРОТОТИПНОГО НАСЛЕДОВАНИЯ ОТ КОНТРОЛЛЕРА!!!
3. ИЗОЛИРОВАННЫЙ СКОУП В ДИРЕКТИВЕ ГАРАНТИРУЕТ ЧТО ВЫ НЕ ПОМЕНЯЕТЕ СЛУЧАЙНО ЗНАЧЕНИЕ
   В КОНТРОЛЛЕРЕ
`
 console.log(str + '\n' + gl_hr);
